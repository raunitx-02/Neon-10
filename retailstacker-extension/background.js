...
async function handleFetchSuggestions(query) {
  try {
    const res = await fetch(`https://completion.amazon.in/api/2017/suggestions?limit=8&prefix=${encodeURIComponent(query)}&alias=aps&site-variant=desktop&version=2`);
    const data = await res.json();
    return data;
  } catch (err) {
    console.error("Background direct fetch-suggestions failed, trying API proxy:", err);
    try {
      const API_BASE = await getApiUrl();
      ...
      console.error("Background fetch-suggestions fallback failed:", e);
      return { suggestions: [] };
    }
  }
}
